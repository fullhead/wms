...
/// Аналогично ReportReceiveTable, только для "issues".
class ReportIssueTable extends StatefulWidget {
  final ReportType reportType;
  const ReportIssueTable({super.key, required this.reportType});

  @override
  ReportIssueTableState createState() => ReportIssueTableState();
}

class ReportIssueTableState extends State<ReportIssueTable> {
  late Future<Map<String, dynamic>> _reportFuture;
  final ReportPresenter _presenter = ReportPresenter();

  // Текущие записи отчёта
  List<ReportEntry> _currentData = [];
  // Полный ответ backend
  ...
  /// Запрашивает данные отчёта для "issues" в зависимости от типа.
  void _loadReport() {
    final now = DateTime.now();
    switch (widget.reportType) {
      case ReportType.daily:
        final dateStr = DateFormat('yyyy-MM-dd').format(now);
        _reportFuture = _presenter.getDailyReport('issues', dateStr);
        break;

      case ReportType.weekly:
        final dateStr = DateFormat('yyyy-MM-dd').format(now);
        _reportFuture = _presenter.getWeeklyReport('issues', dateStr);
        break;

      case ReportType.monthly:
        final year = DateFormat('yyyy').format(now);
        final month = DateFormat('MM').format(now);
        _reportFuture = _presenter.getMonthlyReport('issues', year, month);
        break;
      ...
      case ReportType.interval:
        if (_startDate != null && _endDate != null) {
          ...
          _reportFuture = _presenter.getIntervalReport('issues', start, end);
        } else {
  ...
  /// Позволяет получить текущие записи.
  List<ReportEntry> getCurrentData() => _currentData;

  /// Получает полный ответ бэкенда (для PDF-заголовков и т.д.).
  Map<String, dynamic>? getCurrentResponse() => _currentResponse;

  /// Возвращает тип отчёта (daily, weekly, monthly, interval).
  ...
  /// Используем ту же функцию для формирования человекочитаемого заголовка.
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<Map<String, dynamic>>(
      ...
      builder: (context, snapshot) {
        ...
        // Для "интервального" отчета добавляем выбор дат
        if (widget.reportType == ReportType.interval) {
          return Column(
            children: [
          ...
          // Для дня/недели/месяца — заголовок + таблица
          return Column(
            children: [
              const Divider(height: 1, color: Colors.black),
              Padding(
